...
def checkConsistency(inputDict, currentHttpdConf, start, end):
    ...
    for lineNum in range(start, end):
        ...
        if currentHttpdConf[lineNum].strip().startswith("# PROXYRULE"):
            ...
            if inLine[2] in list(inputDict['nodes'].keys()) and int(inputDict['nodes'][inLine[2]]['port']) == int(inLine[4]):
    ...
    print('Currently there is still %s hosts left' % list(inputDict['nodes'].keys()))
    for ipv4, values in list(inputDict['nodes'].items()):
        changed = True
